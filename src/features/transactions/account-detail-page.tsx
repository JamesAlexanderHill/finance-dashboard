...
import InstrumentCard from '~/features/transactions/components/instrument-card'
...
import EventPreviewTable from '~/features/transactions/components/event/event-preview-table'
...
const getData = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data }) => {
    ...
    if (!session) return { user: null, workspace: null, account: null, accountInstruments: [], recentAccountfiles: [], recentAccountEvents: [], balanceHistory: [], chartInstrument: null, rates: {} as InstrumentRates, annotations: { data: [] } }

    const { ctx, user, workspace } = session
    const account = await accountService.getById(ctx, data.accountId)

    if (!account) return { user, workspace, account: null, accountInstruments: [], recentAccountfiles: [], recentAccountEvents: [], balanceHistory: [], chartInstrument: null, rates: {} as InstrumentRates, annotations: { data: [] } }
...
export const accountDetailLoader = getData

export type AccountDetailPageData = Awaited<ReturnType<typeof getData>>
...
export function AccountDetailPage(props: AccountDetailPageData) {
  const { user, workspace, account, accountInstruments, recentAccountfiles, recentAccountEvents, balanceHistory, chartInstrument, rates, annotations } = props
