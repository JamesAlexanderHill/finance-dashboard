import { Link, Scripts, useRouter, useRouterState } from '@tanstack/react-router'
...
export function Sidebar() {
  ...
  const navLinks = registry.getNavLinks()
  const links = isDev ? [...navLinks, DEV_LINK] : navLinks
  ...
  return (
    <html lang="en">
      ...
      <body className="bg-gray-50 dark:bg-gray-950">
        {showChrome && <Sidebar />}
        <main className={showChrome ? 'ml-56 min-h-screen p-6' : 'min-h-screen'}>{wrapped}</main>
        <Scripts />
      </body>
    </html>
  )
