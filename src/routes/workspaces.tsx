...
function WorkspacesPage() {
  ...
  // Members are managed in local state so add/remove can update instantly.
  const [members, setMembers] = React.useState<Member[]>(() => loaderData.members as Member[])
  const { user, currentWorkspaceId, workspaces } = loaderData
  ...
  async function handleCreate(e: React.FormEvent<HTMLFormElement>) {
    ...
    setShowCreate(false)
    // Invalidate all queries (includes sidebar) then reload route data.
  ...
  async function handleSwitch(workspaceId: string) {
    ...
    // Invalidate all queries so the sidebar workspace switcher updates too.
    await queryClient.invalidateQueries()
    router.invalidate()
  }
        ...
        <div>
          ...
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl">
            ...
            <div className="border-t border-gray-100 dark:border-gray-800">
              ...
              {removeMemberError && (
                <p className="text-xs text-red-600 dark:text-red-400 px-4 pb-3">{removeMemberError}</p>
              )}
              {isOwner && (
